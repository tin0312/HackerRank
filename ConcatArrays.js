...
function getHosts(data) {
    // Use reduce to concatenate arrays of hosts
    return data.reduce((accumulator, currentValue) => 
        accumulator.concat(currentValue.hosts), []);
}
...
function assignAwards(data) {
    ...
    return allHosts.map(host => {
        ...
        const randomPrizeIndex = Math.floor(Math.random() * awards.length);
        const randomPrize = awards[randomPrizeIndex];
        
        // Assign each host with a random prize
        return `${randomPrize} ${host}`;
    });
}
